...
type StepStatus = 'completed' | 'processing' | 'waiting' | 'failed';
  ...
  if (status === 'failed') {
    return (
      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-red-500">
        <IconCozCrossFill className="text-xs text-white" />
      </span>
    );
  }
  return (
...
/** 连接线颜色：取决于其下方节点的状态（已通过蓝色 / 失败红色 / 未到灰色） */
function resolveLineColor(status: StepStatus): string {
  if (status === 'failed') {
    return 'rgba(var(--coze-red-5), 1)';
  }
  if (status === 'completed') {
    return 'rgba(var(--coze-brand-7), 1)';
  }
  return 'rgba(75,74,88,.04)';
}
  ...
  return (
    <div className="mx-auto flex max-w-[1200px] gap-8 pt-12 pb-12">
      ...
      <div className="flex flex-1 flex-col items-center">
        ...
        {/* 步骤列表 */}
        {task.error_message ? (
          <div className="p-4 bg-[rgba(239,68,68,.1)] rounded-[5px] mt-10 ">
            <Typography.Text type="secondary">
              优化失败原因：{task.error_message}
            </Typography.Text>
          </div>
        ) : null}
        <div className="mt-10 w-full max-w-[680px] rounded-lg coz-mg-secondary px-8 py-6">
          {OPTIMIZATION_STEPS.map((step, idx) => {
            ...
            const nextStep = OPTIMIZATION_STEPS[idx + 1];
            const nextStatus = nextStep
              ? resolveStepStatus(nextStep, task.stage)
              : null;
            return (
              <div key={step.id} className="flex">
                ...
                <div className="flex flex-col items-center">
                  ...
                  {idx < OPTIMIZATION_STEPS.length - 1 && nextStatus ? (
                    <div
                      className="w-px flex-1"
                      style={{ backgroundColor: resolveLineColor(nextStatus) }}
                    />
                  ) : null}
                </div>
